...
public class ContainerImplementation implements Container {
	...
	public ContainerImplementation(Map<Class<?>, Class<?>> classesMap, Map<Class<?>, Object> instancesMap) {
	...
	public <T> T getComponent(Class<T> clazz) {
		...
		if (classesMap.containsKey(clazz)) {
			...
			if (implementations.isEmpty()) {
				throw new RuntimeException("Unregistered class: %s".formatted(clazz));
			}

			if (implementations.size() > 1) {
				throw new RuntimeException("Too many implementations for class %s: %s"
	...
	private <T> T getInstance(Class<T> clazz) {
		...
		try {
			...
			throw new RuntimeException("Couldn't instantiate class %s".formatted(clazz));
		}
		return instance;
	}
